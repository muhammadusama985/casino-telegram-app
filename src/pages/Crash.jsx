...
//             {/* Top-left multiplier HUD over the Lottie area (made a bit larger) */}
//             <div className="mult-hud">{fmt(mult)}×</div>
...
//   background: radial-gradient(160px 60px at 20% 0%, rgba(31,41,72,.9) 0%, rgba(10,13,20,.9) 70%);
//   backdrop-filter: blur(6px);
//   font-weight:900;
//   font-size:20px; /* bigger as requested */
//   color:#E5FF8A;
...
export default function Crash() {
  ...
  // Lottie (Girl1 handles rocket): idle → fly → high-fly → straight crash
  const lottieWrapRef = useRef(null);
  const lottieAnimRef = useRef(null);
  ...
  // NEW: track if we've switched into the "≥5x" 18-second animation phase
  ...
  const SEG = useRef({
    IDLE: [0, 120],
    FLY_LOW: [10, 2200],
    FLY_HIGH: [2200, 3280],        // ~18s slice (60 fps → 18s ≈ 1080 frames)
    CRASH_STRAIGHT: [3300, 3600],  // straight + disappear only
  });
