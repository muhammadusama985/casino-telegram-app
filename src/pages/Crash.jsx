...
export default function Crash() {
  ...
  // lane 1 join state
  const [inBet1, setInBet1] = useState(false); // did user join via lane 1?
  const [lockedBet1, setLockedBet1] = useState(0); // frozen stake for lane 1

  // lane 2 join state
  const [inBet2, setInBet2] = useState(false); // did user join via lane 2?
  const [lockedBet2, setLockedBet2] = useState(0); // frozen stake for lane 2
  ...
  const [cashoutAt, setCashoutAt] = useState(null); // global cashout multiplier
  const [roundId, setRoundId] = useState(null);
  const [bustPoint, setBustPoint] = useState(0); // crash multiplier
  const serverSettledRef = useRef(false); // prevent client-side double credit
  ...
  // derived: did we bet this round at all?
  const inBetAny = inBet1 || inBet2;
  ...
  const startFlight = async () => {
    ...
    // Use backend-provided crashAt if joined; otherwise simulate
    const crashAt =
      inBetAny && Number.isFinite(bustPoint) && bustPoint > 1.01
        ? bustPoint
        : randomCrash();
    setBustPoint(crashAt);
  ...
  // Place bet during countdown (optimistic UI) — TWO LANES
  const placeBetLane = async (lane) => {
    const isLane1 = lane === 1;
    const laneInBet = isLane1 ? inBet1 : inBet2;
    const currentBet = isLane1 ? bet1 : bet2;

    if (phase !== "countdown" || laneInBet) return;
    const stake = Math.max(1, Math.floor(Number(currentBet || 0)));
    if (!Number.isFinite(stake) || stake < 1)
      return alert("Enter a valid bet amount.");

    // 🔹 Optimistic UI: lock per-lane bet, mark joined for this lane
    if (isLane1) {
      setLockedBet1(stake);
      setInBet1(true);
    } else {
    ...
    try {
      ...
      if (Number.isFinite(Number(resp?.crashAt))) {
        setBustPoint(Number(resp.crashAt) || 0);
      }
      ...
      // 🔻 If API fails, revert optimistic state for this lane
      if (isLane1) {
        setInBet1(false);
        setLockedBet1(0);
      } else {
  ...
  const placeBet1 = () => placeBetLane(1);
  const placeBet2 = () => placeBetLane(2);

  // Cash out during flight (server-settled, optimistic UI)
  // (applies to all active bets this round; both buttons call same handler)
  const cashoutNow = async () => {
    if (phase !== "running" || !inBetAny || cashoutAt != null) return;
    if (startAtMs && Date.now() < startAtMs) return; // not started yet on server
    ...
    try {
      ...
      if (resp?.ok) {
        ...
        // ✅ tick() sees cashoutAt != null and calls endRound("cashed")
      } else {
        const code = resp?.error || "";
        ...
        if (code === "already-crashed" || code === "too-late") {
          // Backend says it was actually too late → treat as crash instead
          flashToast("Already crashed!", "bad");
          setCashoutAt(null); // cancel optimistic cashout
          playSegment(SEG.current.CRASH_STRAIGHT, true);
          endRound("crashed");
          return;
        }
  ...
  const canCashout1 = phase === "running" && inBet1 && cashoutAt == null;
  const canCashout2 = phase === "running" && inBet2 && cashoutAt == null;
      ...
      <div className="wrap">
        ...
        <div className="panel">
          <div className="card">
            ...
            <div className="bet-ui">
              <div className="bet-left">
                <div className="amount-wrap purple-input">
                  <button
                    className="step purple-step minus"
                  ...
                  <input
                    className="amount-input purple-amount"
                    type="number"
                    min={1}
                    step="1"
                  ...
                  <button
                    className="step purple-step plus"
                ...
                <div className="quick-row">
                  <button
                    className="quick purple-chip"
                    disabled={phase !== "countdown" || inBet1}
                    onClick={() =>
                      setBet1((b) => Math.max(1, Math.floor(b / 2)))
                    }
                  >
                    /2
                  </button>
              ...
              <div className="bet-right">
                {phase === "countdown" && !inBet1 && (
                  <button
                    className={`bet-cta neon-cta ${
                      !canPlaceBet1 ? "disabled" : ""
                    }`}
                ...
                {phase === "countdown" && inBet1 && (
                  <button className="bet-cta neon-cta disabled" disabled>
                    Joined · {countdown}s
                  </button>
                )}
                {running && (
            ...
            {/* === LANE 2 (same design) === */}
            <div className="bet-ui bet-ui-second">
              <div className="bet-left">
                <div className="amount-wrap purple-input">
                  <button
                    className="step purple-step minus"
                ...
                <div className="quick-row">
                  <button
                    ...
                    disabled={phase !== "countdown" || inBet2}
                    onClick={() =>
                      setBet2((b) => Math.max(1, Math.floor(b / 2)))
                    }
                  >
                    /2
                  </button>
            ...
            {(crashed || cashed) && (
              <div className={`round-result ${crashed ? "bad" : "good"}`}>
                {crashed ? (
                  <>
                    Busted at <b>{fmt(bustPoint)}×</b>.
                    {!inBetAny ? " You didn’t join this round." : " Your stake was lost."}
                  </>
                ) : (
                  <>
                    Cashed at <b>{fmt(cashoutAt)}×</b>. Payout credited.
...
.bet-cta { 
  ...
  color:#fff;
  background:linear-gradient(135deg,#5F34FF 0%,#B13CFF 50%,#FF4FD2 100%); 
  box-shadow:
    0 14px 28px rgba(80,30,255,0.55),
...
.bet-cta::after {
  ...
  height:6px;
  border-radius:999px;
  background:linear-gradient(90deg,#8F5BFF,#FF6EEC);
  opacity:.85;
  filter:blur(4px);
}

.bet-cta:hover:not(.disabled) { 
  filter:brightness(1.08); 
  box-shadow:
    0 18px 38px rgba(80,30,255,0.8),
    0 0 0 1px rgba(255,255,255,0.10);
}

.bet-cta:active:not(.disabled) { 
...
/* purple-styled controls to match the screenshot */
.purple-input {
  background:#14122A;
  border-color:#312250;
}
...
.purple-step:hover:not(:disabled) {
  background:#2A2152;
}
...
.purple-chip:hover:not(:disabled) {
  background:#241F46;
}
