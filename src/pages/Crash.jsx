...
/************ CSS (moved out so it’s not recreated every render) ************/
const CRASH_STYLES = `
        .countdown-main { fill:#ffffffdd; font-size:80px; font-weight:900; filter:url(#softGlow); }
        .countdown-sub { fill:#ffffff99; font-size:20px; font-weight:600; }
        .bet-cta.disabled { opacity:0.6; cursor:not-allowed; }
...
export default function Crash() {
  ...
  // live frame state
  const [multDisplay, setMultDisplay] = useState(1); // shown in HUD and button
  const multRef = useRef(1); // true multiplier used for logic & smooth animation
  ...
  function tick(ts) {
    ...
    if (elapsed >= tEnd - 1e-6) {
      if (bustPoint && Number.isFinite(bustPoint)) {
        multRef.current = bustPoint;
        setMultDisplay(bustPoint); // final snap for HUD
      }
    ...
    // store current multiplier only in ref (no React re-render)
    multRef.current = m;
  ...
  useEffect(() => {
    ...
    const step = () => {
      ...
      const target = startAtMs ?? localCountdownEndRef.current ?? now;
      const leftMs = Math.max(0, target - now);
      setCountdown(Math.min(999, Math.ceil(leftMs / 1000)));
      if (leftMs <= 0) {
        startFlight();
      } else {
  ...
  // Smoothly update the displayed multiplier from the ref (decoupled from main animation)
  useEffect(() => {
    let frameId;
    let lastTs = 0;
    ...
    const updateDisplay = (ts) => {
      if (!lastTs) lastTs = ts;
      // update UI about ~12 times per second (≈80 ms)
      if (ts - lastTs >= 80) {
        lastTs = ts;
        const m = multRef.current;
        // only trigger render if the 2-decimal value changed
        setMultDisplay((prev) =>
          Math.round(prev * 100) === Math.round(m * 100) ? prev : m
        );
      }
  ...
  const cashoutNow = async () => {
    ...
    // Use the true current multiplier from ref, rounded to 2 decimals
    const x = round2(multRef.current);
    try {
      const resp = await crashCashout({ roundId, x });
      if (resp?.ok) {
        serverSettledRef.current = true;
      ...
      <div className="wrap">
        ...
        <div className="graph-card">
          {/* <div className="graph-title">
            ...
            {showMult && <div className="mult">{fmt(multDisplay)}×</div>}
          </div> */}
          ...
          <div className="graph-area">
            ...
            {/* Top-left multiplier HUD over the Lottie area (larger) */}
            {showMult && <div className="mult-hud">{fmt(multDisplay)}×</div>}
