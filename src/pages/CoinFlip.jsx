...
const Coin3D = forwardRef(function Coin3D({ ariaFace = "H" }, ref) {
  ...
  useImperativeHandle(ref, () => ({
    // Begin Lottie-like motion while waiting for backend
    startWaiting() {
      const el = coinRef.current;
      if (!el) return;
      clearTimers();
      ...
      // eslint-disable-next-line no-unused-expressions
      el.offsetWidth;
      el.classList.add("coinflip-lottie"); // <-- new: lottie-inspired loop
    },
    // Resolve to 'H' or 'T' and return a Promise that fulfills after the animation ends
    resolve(desired) {
      const el = coinRef.current;
      ...
      // stop lottie loop and run deterministic settle
      el.classList.remove("coinflip-lottie");
  ...
  return (
    <div className="coinflip-scene" style={{ perspective: "1200px" }}>
      ...
      <style>{`
        ...
        /* ---------- Lottie-like waiting loop (bounce + spin) ---------- */
        .coinflip-lottie {
          animation: coinflip-lottie-motion 18s linear infinite;
        }
        @keyframes coinflip-lottie-motion {
          /* combined rotateX + translateY to mirror the Lottie beats */
          0%   { transform: rotateX(0deg)     rotateY(-6deg) rotateZ(3deg) translateY(0px); }
          5.56%{ transform: rotateX(200deg)   rotateY(-6deg) rotateZ(3deg) translateY(0px); }
          11.11%{transform: rotateX(400deg)   rotateY(-6deg) rotateZ(3deg) translateY(-47px); }  /* lift */
          12.5%{ transform: rotateX(450deg)   rotateY(-6deg) rotateZ(3deg) translateY(0px); }    /* baseline */
          13.89%{transform: rotateX(500deg)   rotateY(-6deg) rotateZ(3deg) translateY(18px); }   /* overshoot */
          15.28%{transform: rotateX(550deg)   rotateY(-6deg) rotateZ(3deg) translateY(0px); }    /* settle */
          16.67%{transform: rotateX(600deg)   rotateY(-6deg) rotateZ(3deg) translateY(0px); }

          33.33%{transform: rotateX(1200deg)  rotateY(-6deg) rotateZ(3deg) translateY(0px); }
          50%   {transform: rotateX(1800deg)  rotateY(-6deg) rotateZ(3deg) translateY(0px); }
          66.67%{transform: rotateX(2400deg)  rotateY(-6deg) rotateZ(3deg) translateY(0px); }
          83.33%{transform: rotateX(3000deg)  rotateY(-6deg) rotateZ(3deg) translateY(0px); }
          100%  {transform: rotateX(3600deg)  rotateY(-6deg) rotateZ(3deg) translateY(0px); }   /* ~10 flips total */
        }
        ...
        @media (prefers-reduced-motion: reduce) {
          .coinflip-lottie { animation-duration: 0.01ms; }
          .coinflip-coin.coinflip-anim { animation-duration: 0.01ms; }
        }
      `}</style>
    </div>
